...
- **Architecture** (`ie-architecture-reviewer`) — framework structure (Rails + Python +
  Laravel + Express + Phoenix today). Fat models/routers/controllers/handlers, God
  objects/modules/contexts, misused service objects, callback hell, business logic in
  schemas/changesets, queries in views / N+1, layer leaks, async/process misuse, Law of
  Demeter. Classifies design-pattern instances against a per-stack catalog, raises
  ...
  (Python), `phpstan`/`phpmd` (Laravel), `eslint`/`madge` (Express), or `credo`/`boundary`
  (Phoenix) if installed. Code/audit only, when a supported framework is detected.
...
- `resources/frameworks/` — per-stack conventions (Rails, Ruby, React, TypeScript,
  Python, Laravel, Express, Phoenix, Swift/iOS).
...
- `resources/frameworks/{rails,python,laravel,express,phoenix}-architecture.md` + `resources/patterns/{rails,python,laravel,express,phoenix}.yaml`
